...
@Form.create()
class UserLogList extends PureComponent {
  state = {
    modalVisible: false,
  ...
  columns = [
    ...
    },
    {
      ...
      render: (text, record) => (
        <Fragment>
          <a onClick={() => this.handleDownloadFile(record)}>下载</a>
        </Fragment>
      ),
    },
  ];
  ...
  handleDownloadFile = (record) => {
    ...
    dispatch({
      type: 'userloglist/downloadFile',
      payload: record.name,
    });
    console.log("handleDownloadFile" + record.name);
  };
